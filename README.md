# Сборка
1. git clone -b 'hw14' --single-branch https://github.com/etrehappy/ue_prj_a.git
2. Скачать [архив (416 MB)](https://drive.google.com/file/d/1s1it7b2EnF0ybD_9knUfaOV0yhE38z7G/view?usp=sharing) и распаковать с заменой в .\ue_prj_a\ 
...
- [ ] ~~NPC обладают базовыми способностями персонажа игрока~~
    ...
    - добавлен оружейный компонент для NPC [UNpcBattleComponent](./Project_A/Source/Npc/Public/NpcBattleComponent.h);
    - реализована логика атаки ближним оружием у бота-гоблина;
    - NPC может переиспользовать компоненты, созданные ранее для игрока (например, инвентарь и [компонент здоровья](./Project_A/Source/Npc/Public/NpcBase.h));
...
2. Разное поведение NPC:
    - [NpcAIController](./Project_A/Source/Npc/Public/NpcAIController.h) использует поле *StateTreeAIComponent*. Созданы несколько деревьев для разных ботов: 
...
4. Квестовая система: 
    ...
    - класс [AQuestNpcBase](./Project_A/Source/Quest/Public/QuestNpcBase.h) расширяет функционал NPC для взаимодействия с ним и получения квеста;
    - класс [AQuestObelisk](./Project_A/Source/Quest/Public/FirstQuest/QuestObelisk.h) отвечает за объект, с которыми можно будет взаимодействовать, чтобы выполнить квест;
    ...
    - класс [UQuestLogComponent](./Project_A/Source/Quest/Public/QuestLogComponent.h) отвечает за квестовую логику у игрока; 
