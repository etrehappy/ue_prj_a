# Сборка
1. git clone -b 'hw21' --single-branch https://github.com/etrehappy/ue_prj_a.git
2. Скачать [архив (1,26 Гб)](https://drive.google.com/file/d/1CiLtmP6KIoYi2C_fEaQPbkvb2pImfQ2j/view?usp=sharing) и распаковать с заменой в .\ue_prj_a\ 
...
На github загружен только минимум, в Project_A.Build.cs можно закомментировать лишние пути, а в Project_A.uproject — отлкючить лишние плагины.

# Запуск проекта
Проект разделён на клиент и серверы, поэтому есть bat-файлы для запуска.
1. В файлах Editor_StartGame и Editor_StartTwoServers исправить пути до UnrealEditor.exe
<br><br>
2. Запустить hub-сервер и игровой сервер через <i>Editor_StartTwoServers.bat</i>
3. Запустить сервер авторизации и сервер с персонажами пользователя <i>StartHelperServers.bat</i>
4. Запустить клиент через <i>Editor_StartGame.bat</i>
5. Открыть виджет авторизации через "Play MMO"
    - логин: player1
    - пароль: 1234 
6. Подключиться к hub-серверу, выбрать сервер, персонажа и войти на игровой сервер
...
- <i>Нет конфигурируемости через .ini для подключения к серверу</i> — добавлено поле LobbyTargetWorldAddress в [LobbyGameMode](https://github.com/etrehappy/ue_prj_a/blob/hw19/Project_A/Source/Project_A/Public/Core/LobbyGameMode.h) и соответсвующее поле [DefaultNetSetCustom.ini](https://github.com/etrehappy/ue_prj_a/blob/hw19/Project_A/Config/DefaultNetSetCustom.ini)
...
**Как сделано**:
1. Лобби
    - Добавлены: [LobbyGameMode](https://github.com/etrehappy/ue_prj_a/blob/hw19/Project_A/Source/Project_A/Public/Core/LobbyGameMode.h), [LobbyGameState](https://github.com/etrehappy/ue_prj_a/blob/hw19/Project_A/Source/Project_A/Public/Core/LobbyGameState.h), [LobbyPlayerState](https://github.com/etrehappy/ue_prj_a/blob/hw19/Project_A/Source/Project_A/Public/Core/LobbyPlayerState.h), [LobbyPlayerController](https://github.com/etrehappy/ue_prj_a/blob/hw19/Project_A/Source/Project_A/Public/Character/LobbyPlayerController.h), [LobbyHud](https://github.com/etrehappy/ue_prj_a/blob/hw19/Project_A/Source/UI/Public/LobbyHud.h), 
        - [LobbyWidget](https://github.com/etrehappy/ue_prj_a/blob/hw19/Project_A/Source/UI/Public/LobbyMainWidget.h) — главный экран при подключении к Lobby-серверу, 
        - [LobbyRoomWidget](https://github.com/etrehappy/ue_prj_a/blob/hw19/Project_A/Source/UI/Public/LobbyRoomWidget.h) — отдельная комната, в которой можно выбрать персонажа и начать игру.
...
2. Подключение к игровой сцене
    - выполняется в ALobbyGameMode::[StartMatchTravelForRoom](https://github.com/etrehappy/ue_prj_a/blob/hw19/Project_A/Source/Project_A/Public/Core/LobbyGameMode.cpp). Вызывается, если обратный отсчет завершился в ALobbyGameMode::HandleRoomCountdownTick или вызван ALobbyGameMode::RequestForceStart.

3. Взаимодействие
    - атака была реализована в [задании 7](#task7) и урон по здоровью в [задании 8](#task8);
    - объединение в группу реализовано с помощью 
        - [IInteractable](https://github.com/etrehappy/ue_prj_a/blob/hw19/Project_A/Source/Interaction/Public/Interactable.h) — добавлены методы BuildInteractionActions и ExecuteInteractionAction, игрок теперь тоже реализует IInteractable (объект, с которым можно взаимодействовать).
        - [InteractionActionType](https://github.com/etrehappy/ue_prj_a/blob/hw19/Project_A/Source/Interaction/Public/InteractionActionType.h) — определние того, какие действия можно совершать с объектом.
        - [NetPlayerState](https://github.com/etrehappy/ue_prj_a/blob/hw19/Project_A/Source/Interaction/Public/NetPlayerState.h) — пока хранит только идентификатор группы. 
        - через AGeneralHud::[ShowInteractionMenu](https://github.com/etrehappy/ue_prj_a/blob/hw19/Project_A/Source/UI/Public/GeneralHud.h) отображается список действий.
        - С помощью ANetPlayerCharacter::[ExecuteInteractionAction](https://github.com/etrehappy/ue_prj_a/blob/hw19/Project_A/Source/Project_A/Private/Character/NetPlayerCharacter.cpp) выполнятеся доступное действие. 

</details>


<details><summary id="task21">Задание 21. Проектная работа<p></p></summary>
...
- [x] игровой персонаж полноценно функционирует;
- [x] в игре имеются NPC;
- [x] игра проходится от начала до конца (короткая демо-игра не менее чем на 10 мин.);
- [x] игровой код выполнен без критических ошибок (игра не вылетает).


Исправления:
- Добавлена простая [подсистема авторизации пользователя](./Project_A/Source/Project_A/Public/Core/AuthService.h). После успешной авторизации [GameInstanceBase](./Project_A/Source/Project_A/Public/GameInstanceBase.h) хранит AccountId и SessionToken. Далее выполняется подключение к hub-серверу через AClientStartGameMode::[ConnectToHubServer()](./Project_A/Source/Project_A/Private/Core/ClientStartGameMode.cpp). [Сервер авторизации](./AuthServer/auth_server.py) сгенерирован на python. 
- Добавлена простая [подсистема хранения данных о персонажах игрока](./Project_A/Source/Project_A/Public/CharacterService.h). На hub-сервере пользователь получает список своих персонажей из UCharacterService::[FetchCharacterList()](./Project_A/Source/Project_A/Private/CharacterService.cpp). После выбора персонажа и сервера пользователь подключается к игровому миру через AHubGameMode::EnterToWorldWithCharacter(./Project_A/Source/Project_A/Private/Core/HubGameMode.cpp). [Сервер для хранения данных персонажа](./CharacterServer/character_server.py) (сгенерирован на python). 
- Через [AWorldGameMode::LoadAndApplyCharacterData и AWorldGameMode::InitNewPlayer](./Project_A/Source/Project_A/Private/Core/WorldGameMode.cpp) пользователь и персонаж загружаются в игровом мире.
- Через [AWorldGameMode::Logout()](./Project_A/Source/Project_A/Private/Core/WorldGameMode.cpp) сохраняются некоторые данные персонажа при выходе из игры (проверялись только местоположение и имя после возвращения в мир).
- В [задании 14](#task14) была сделана заглушка для квестовой системы. Теперь она полностью пересмотрена.
- у дружественных NPC и врагов можно открыть инвентарь;
- Сетевые настройки (адреса) вынесены из header-файлов в конфиг: [DefaultGame.ini](./Project_A/Config/DefaultGame.ini) и [DefaultNetSetCustom.ini](./Project_A/Config/DefaultNetSetCustom.ini)

Дополнительно: 
- добавлена диалоговая система; 
- добалены минимальные звуки (окружение + атака для fireball).

**Как сделано**:
1. Диалоговая система:
    - flow для удобства описан в [DialogueManagerSubsystem.h](./Project_A/Source/Dialogue/Public/DialogueManagerSubsystem.h);
    - коротко: диалог - это DataAsset c узлами (речью NPC) и выборами для игрока. Узлы и выборы имеют входные условия и оказываемые эффекты (например, изменить статус квеста или открыть инвентарь). 
    - эффекты применяются через UDialogueManagerSubsystem::[ApplyEffects()](./Project_A/Source/Dialogue/Private/DialogueManagerSubsystem.cpp)
    - реализован диалог только для 1 NPC (/Game/Project_A_Root/Dialogue/DA_TestDialogue)
2. Квестовая система:
    - реализована с помощью [UQuestManagerSubsystem](./Project_A/Source/Quest/Public/QuestManagerSubsystem.h) и [UQuestLogComponent](./Project_A/Source/Quest/Public/QuestLogComponent.h) для игрока;
    - тело квеста описывается с помощью DataAsset и имеет подзадачи(например, /Game/Project_A_Root/Quests/DA_QuestObelisk);
    - Name-поле QuestId в описании квеста и его статус выполнения используется в диалогах и интерактивных предметах, чтобы выдать\принять задание или получить доступ к объекту. Интекрактивные предметы для этого используют [QuestObjectiveSourceComponent.h](./Project_A/Source/Quest/Public/QuestObjectiveSourceComponent.h)
        <div style="text-align: center;">
            <img src="./img_for_readme/quest_obj.jpg" alt="quest_obj" width="500" height="227">   
        </div>
