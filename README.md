...
2. Скачать [архив (416 MB)](https://drive.google.com/file/d/1SA74Gd3HMtEOnHNvXb4e9CxvSkq4EFHo/view?usp=sharing) и распаковать с заменой в .\ue_prj_a\ 
...
2. Cмена оружия через инвентарь
    - начинается с события UInventorySlotWidget::NativeOnDragDetected(./Project_A/Source/UI/Private/InventoryWidget.cpp) (перемещение предметов внутри инвентаря реализовано в задинии 10, [см. выше](#task10) ); 
...
3. Ячейка с бомбами:
    ...
    - в виджет экипированных предметов [UEquippedItemWidget](./Project_A/Source/UI/Public/EquippedItemWidget.h) добавлено поле ThrowableSlot для метательных предметов; 
    - ячейка поддерживает стак в отличие от ячейки для обычного оружия; 
    - после успешного броска UCombatComponent::[OnSpawnBombProjectile()](./Project_A/Source/Project_A/Private/Character/CombatComponent.cpp) кол-во бомб в инвентаре уменьшается за счёт UInventory::[UseItemFromSlot()](./Project_A/Source/Inventory/Private/Inventory.cpp).

4. Сброс активного оружия после перемещения в инвентарь
    - [UCombatComponent](./Project_A/Source/Project_A/Private/Character/CombatComponent.cpp) подписан на изменение инвентаря: если оружие в руке персонажа и его удаляют из ячейки, то [HandleEquipmentChanged()](./Project_A/Source/Project_A/Private/Character/CombatComponent.cpp) сбросит состояние к обычному.  
</details>
